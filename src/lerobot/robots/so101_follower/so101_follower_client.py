...
class SO101FollowerClient(Robot):
    ...
    def __init__(self, config: SO101FollowerClientConfig) -> None:
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "SO101 Follower Client is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for SO101 Follower Host to connect expired.")
    ...
    def get_observation(self) -> dict[str, Any]:
        """
        Capture observations from the remote robot: current follower arm positions and a camera frame. 
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "LeRobot is not connected. You need to run `robot.connect()` before disconnecting."
            )
